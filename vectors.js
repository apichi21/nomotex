...
function initShadersTextureLight() {
    ...
    newShaderProgram.vertexTextureAttribute = gl.getAttribLocation(newShaderProgram, "aTextureCoord");
    gl.enableVertexAttribArray(newShaderProgram.vertexTextureAttribute);
    ...
    newShaderProgram.samplerUniform = gl.getUniformLocation(newShaderProgram, "uSampler");
    ...
    newShaderProgram.bindBuffers = function(buffers) {
        ...
        gl.vertexAttribPointer(this.vertexTextureAttribute, buffers.vertexTexture.itemSize, gl.FLOAT, false, 0, 0);
    }
    shaderProgram1=newShaderProgram;
}
...
function initAddAxes() {
    ...
    var rad=1;
    mass=initArrow([xLeft,0,0], [xCount-1+xLeft+1,0,0], rad, [0.3, 0.3, 0.3, 1.0],indiceBase,false,true,true);
    ...
    mass=initArrow([0,yDown,0], [0,(yCount-1+yDown+1),0], rad, [0.3, 0.3, 0.3, 1.0],indiceBase,false,true,true);
    Array.prototype.push.apply(vertices, mass[0]);
    Array.prototype.push.apply(colors, mass[1]);
    Array.prototype.push.apply(indices, mass[2]);
    Array.prototype.push.apply(zoomvectors, mass[3]);
    ...
    mass=initArrow([0,0,zFar], [0,0,(zCount-1+zFar+1)], rad, [0.3, 0.3, 0.3, 1.0],indiceBase,false,true,true);
    Array.prototype.push.apply(vertices, mass[0]);
    Array.prototype.push.apply(colors, mass[1]);
    Array.prototype.push.apply(indices, mass[2]);
    Array.prototype.push.apply(zoomvectors, mass[3]);
...
function initAxes() {
    ...
    if(is2d) {
        ...
        var rad=1;
        mass=initArrow([objX0Y0[0],0,0], [objWH[0],0,0], rad, [0.3, 0.3, 0.3, 1.0],indiceBase,false,true,true);
        Array.prototype.push.apply(vertices, mass[0]);
        Array.prototype.push.apply(colors, mass[1]);
        Array.prototype.push.apply(indices, mass[2]);
        Array.prototype.push.apply(zoomvectors, mass[3]);
        ...
        mass=initArrow([0,objWH[1],0], [0,objX0Y0[1],0], rad, [0.3, 0.3, 0.3, 1.0],indiceBase,false,true,true);
        Array.prototype.push.apply(vertices, mass[0]);
        Array.prototype.push.apply(colors, mass[1]);
        Array.prototype.push.apply(indices, mass[2]);
        Array.prototype.push.apply(zoomvectors, mass[3]);
        ...
        if (isAxisNumbers) {
            vec3.set([gridStep,0,0], textLabels[2].objCoords);
            textLabels[2].text.html("<span id='axisNumber'>"+parseFloat(gridStep.toPrecision(3))+"</span>");
            textLabels[2].align = "ct";
            vec3.set([0,gridStep,0], textLabels[3].objCoords);
            textLabels[3].text.html("<span id='axisNumber'>"+parseFloat(gridStep.toPrecision(3))+"</span>");
            textLabels[3].align = "rc";
        }
        ...
        var rad=1;
        mass=initArrow([xLeft,0,0], [xRight,0,0], rad, [0.3, 0.3, 0.3, 1.0],indiceBase,false,true,true);
        Array.prototype.push.apply(vertices, mass[0]);
        Array.prototype.push.apply(colors, mass[1]);
        Array.prototype.push.apply(indices, mass[2]);
        Array.prototype.push.apply(zoomvectors, mass[3]);
        ...
        mass=initArrow([0,yDown,0], [0,yUp,0], rad, [0.3, 0.3, 0.3, 1.0],indiceBase,false,true,true);
        Array.prototype.push.apply(vertices, mass[0]);
        Array.prototype.push.apply(colors, mass[1]);
        Array.prototype.push.apply(indices, mass[2]);
        Array.prototype.push.apply(zoomvectors, mass[3]);
        ...
        mass=initArrow([0,0,zFar], [0,0,zNear], rad, [0.3, 0.3, 0.3, 1.0],indiceBase,false,true,true);
        Array.prototype.push.apply(vertices, mass[0]);
        Array.prototype.push.apply(colors, mass[1]);
        Array.prototype.push.apply(indices, mass[2]);
        Array.prototype.push.apply(zoomvectors, mass[3]);
...
function initArrowBuffers() {
    ...
    for (var i = 0; i < primitives.length; i++) {
        if (!primitives[i].offset) {
            ...
            {
                if (useBSPTreeSorting && primitives[i].color[3] < 1 && (primitives[i].class==="plane" || primitives[i].class==="triangle")) {
        ...
        if (!primitives[i].hasOwnProperty('scaling')) primitives[i].scaling = true;

        if (primitives[i].class==="arrow") {
            mass=initArrow(primitives[i].arr0, primitives[i].arr1, primitives[i].rad, primitives[i].color, bufarray.indiceBase,false,true, primitives[i].scaling);
        } else if (primitives[i].class==="darrow") {
            mass=initArrow(primitives[i].arr0, primitives[i].arr1, primitives[i].rad, primitives[i].color, bufarray.indiceBase,true,true, primitives[i].scaling);
        } else if (primitives[i].class==="line") {
            mass=initArrow(primitives[i].arr0, primitives[i].arr1, primitives[i].rad, primitives[i].color, bufarray.indiceBase,false,false, primitives[i].scaling);
            ...
            mass=initSphere(primitives[i].arr0, primitives[i].rad, primitives[i].color, bufarray.indiceBase, primitives[i].scaling);
    ...
    for (var i = 0; i < baseNodesNum; i++) {
        ...
        newNode = {name: "n"+i, parent:null, childB:null, childF:null, neighbours:[], normal:norm, coords:[c0,c1,c2],
    ...
    bindBufData(arrowBuffersTransluent, bufdatatransluent);
        ...
        //console.log("");
    // printNode(treeBSP.root);

    // function printNode(node) {
    //     if (node) {
    //         console.log(node.name, node.neighbours.length);
    //         printNode(node.childB);
    //         printNode(node.childF);
    //     }
    // }
    ...
    // var mineps = 0.0;
    var mineps = 1.0e-10;
        //console.log("addNode(",newNode.name,newNode.coords,parentNode.name,")");
        var cnode;
        var dot = [];
        for (var i = 0; i < 3; i++) {
        ...
        var infront = (dot[0] > mineps) + (dot[1] > mineps) + (dot[2] > mineps);
        var behind = (dot[0] < -mineps) + (dot[1] < -mineps) + (dot[2] < -mineps);
        if (infront > 0 && behind > 0) {
            if ((dot[0] > mineps && dot[1] < -mineps && dot[2] < -mineps) ||
                (dot[0] < -mineps && dot[1] > mineps && dot[2] > mineps)) {
                var v01 = [];
                vec3.subtract(newNode.coords[1], newNode.coords[0], v01);
                var dotSum = Math.abs(dot[0])+Math.abs(dot[1]);
                vec3.scale(v01, Math.abs(dot[0])/dotSum);
                vec3.add(v01, newNode.coords[0]);
                ...
                var v02 = [];
                vec3.subtract(newNode.coords[2], newNode.coords[0], v02);
                var dotSum = Math.abs(dot[0])+Math.abs(dot[2]);
                vec3.scale(v02, Math.abs(dot[0])/dotSum);
                vec3.add(v02, newNode.coords[0]);
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[0,0]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[newNode.coords[0],v01,v02],
                                 indices:[newNode.indices[0], newIndex1, newIndex2]};
                if (dot[0] > mineps) {
                    if (!parentNode.childF) {
                        //console.log("new childF");
                        parentNode.childF = newHalfNode;
                        newHalfNode.parent = parentNode;
                        //console.log("newNode(",newHalfNode.name,newHalfNode.coords,parentNode.name,")");
                    } else {
                        //console.log("add childF");
                        addNode(newHalfNode, parentNode.childF)
                    }
                } else {
                    if (!parentNode.childB) {
                        //console.log("new childB");
                        parentNode.childB = newHalfNode;
                        newHalfNode.parent = parentNode;
                        //console.log("newNode(",newHalfNode.name,newHalfNode.coords,parentNode.name,")");
                    } else {
                        //console.log("add childB");
                        addNode(newHalfNode, parentNode.childB)
                    }
                }

                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[0,1]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[v01,newNode.coords[1],newNode.coords[2]],
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[0,2]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[v01,newNode.coords[2],v02],
            ...
            } else if ((dot[0] < -mineps && dot[1] > mineps && dot[2] < -mineps) ||
                (dot[0] > mineps && dot[1] < -mineps && dot[2] > mineps)) {
                var v10 = [];
                vec3.subtract(newNode.coords[0], newNode.coords[1], v10);
                var dotSum = Math.abs(dot[1])+Math.abs(dot[0]);
                vec3.scale(v10, Math.abs(dot[1])/dotSum);
                vec3.add(v10, newNode.coords[1]);
                var newIndex1 = bufdatatransluent.vertices.length / 3;
                bufdatatransluent.indices.push(0,0,0,0,0,0);
                ...
                var v12 = [];
                vec3.subtract(newNode.coords[2], newNode.coords[1], v12);
                var dotSum = Math.abs(dot[1])+Math.abs(dot[2]);
                vec3.scale(v12, Math.abs(dot[1])/dotSum);
                vec3.add(v12, newNode.coords[1]);
                var newIndex2 = bufdatatransluent.vertices.length / 3;
                // bufdatatransluent.indices.push(newIndex2);
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[1,0]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[v10,newNode.coords[1],v12],
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[1,1]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[newNode.coords[0],v10,newNode.coords[2]],
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[1,2]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[newNode.coords[2],v10,v12],
                ...
                (dot[0] > mineps && dot[1] > mineps && dot[2] < -mineps)) {
                var v20 = [];
                vec3.subtract(newNode.coords[0], newNode.coords[2], v20);
                var dotSum = Math.abs(dot[2])+Math.abs(dot[0]);
                vec3.scale(v20, Math.abs(dot[2])/dotSum);
                vec3.add(v20, newNode.coords[2]);
                var newIndex1 = bufdatatransluent.vertices.length / 3;
                bufdatatransluent.indices.push(0,0,0,0,0,0);
                ...
                var v21 = [];
                vec3.subtract(newNode.coords[1], newNode.coords[2], v21);
                var dotSum = Math.abs(dot[2])+Math.abs(dot[1]);
                vec3.scale(v21, Math.abs(dot[2])/dotSum);
                vec3.add(v21, newNode.coords[2]);
                var newIndex2 = bufdatatransluent.vertices.length / 3;
                // bufdatatransluent.indices.push(newIndex2);
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[2,0]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[v20,v21,newNode.coords[2]],
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[2,1]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[newNode.coords[0],newNode.coords[1],v20],
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[2,2]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[newNode.coords[1],v21,v20],
            ...
            } else if ((dot[0] >= -mineps && dot[0] <= mineps)) {
                var v12 = [];
                vec3.subtract(newNode.coords[2], newNode.coords[1], v12);
                var dotSum = Math.abs(dot[1])+Math.abs(dot[2]);
                vec3.scale(v12, Math.abs(dot[1])/dotSum);
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[0,-0.5]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[newNode.coords[0],newNode.coords[1],v12],
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[0,0.5]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[newNode.coords[0],newNode.coords[2],v12],
            ...
            } else if ((dot[1] >= -mineps) && (dot[1] <= mineps)) {
                var v02 = [];
                vec3.subtract(newNode.coords[2], newNode.coords[0], v02);
                var dotSum = Math.abs(dot[0])+Math.abs(dot[2]);
                vec3.scale(v02, Math.abs(dot[0])/dotSum);
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[1,-0.5]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[newNode.coords[0],newNode.coords[1],v02],
                                 indices:[newNode.indices[0], newNode.indices[1], newIndex2]};
                if (dot[0] > mineps) {
                    if (!parentNode.childF) {
                        //console.log("new childF");
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[1,0.5]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[newNode.coords[2],newNode.coords[1],v02],
            ...
            } else if ((dot[2] >= -mineps) && (dot[2] <= mineps)) {
                var v01 = [];
                vec3.subtract(newNode.coords[1], newNode.coords[0], v01);
                var dotSum = Math.abs(dot[0])+Math.abs(dot[1]);
                vec3.scale(v01, Math.abs(dot[0])/dotSum);
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[2,-0.5]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[newNode.coords[0],v01,newNode.coords[2]],
                                 indices:[newNode.indices[0], newIndex1, newNode.indices[2]]};
                if (dot[0] > mineps) {
                    if (!parentNode.childF) {
                        //console.log("new childF");
                ...
                var newHalfNode = {name: newNode.name+"_"+parentNode.name+"[2,0.5]", parent:null, childB:null, childF:null, neighbours:[], normal:newNode.normal, coords:[newNode.coords[1],v01,newNode.coords[2]],
            ...
            if (!parentNode.childF) {
                ...
                newNode.parent = parentNode;
                        //console.log("newNode(",newNode.name,newNode.coords,parentNode.name,")");
            } else {
                        //console.log("add childF");
...
function initMeshBuffers() {
    for (var i = 0; i < meshes.length; i++) {
        ...
        // console.log("texcoords",meshes[i].texcoords);
            if (meshes[i].texcoords) {
                meshBuffers[i].vertexTexture = gl.createBuffer();
                meshBuffers[i].vertexTexture.itemSize = 2;
                gl.bindBuffer(gl.ARRAY_BUFFER, meshBuffers[i].vertexTexture);
                gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(meshes[i].texcoords), gl.STATIC_DRAW);
            }
        ...
        // console.log(meshes[i].texture);
        if (meshes[i].texture) {
            meshBuffers[i].texture = gl.createTexture();
            meshBuffers[i].texture.image = meshes[i].texture;

            // gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
            gl.bindTexture(gl.TEXTURE_2D, meshBuffers[i].texture);
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, meshBuffers[i].texture.image);
        ...
        if (meshes[i].texcoords) {
            meshBuffers[i].texcoords = meshes[i].texcoords;
        }

        // console.log(meshBuffers[i]);
    }
    ...
    if (meshBuffers.length > meshes.length) {
        for (var i = meshes.length; i < meshBuffers.length; i++) {
            ...
            if (meshBuffers[i].texcoords) {
                gl.deleteBuffer(meshBuffers[i].texcoords);
            }
...
function initLabels() {
    ...
    function midPointAngle(arr0, arr1, arr2, rad, ratio)
    {
        var ppp1 = [];
        var ppp2 = [];
        var ppp = [];
        ...
        vec3.normalize(ppp);
        vec3.scale(ppp, rad*ratio);
        vec3.add(ppp,arr0);

        return ppp;
    }
    function midPointAngleOut(arr0, arr1, arr2, rad, ratio)
    {
        var ppp1 = [];
        var ppp2 = [];
        var ppp = [];
    ...
    for (var i = 0; i < primitives.length; i++) {
        if (primitives[i].text != ''){
            if (primitives[i].class==="arrow") {
                ...
                if (!primitives[i].hasOwnProperty('ratio')) primitives[i].ratio = 3.0/4.0;
                appendLabels(primitives[i].text, midPointAngle(primitives[i].arr0, primitives[i].arr1, primitives[i].arr2, primitives[i].Rad, primitives[i].ratio), 'cc');
            } else if (primitives[i].class==="arcout") {
                if (!primitives[i].hasOwnProperty('ratio')) primitives[i].ratio = 1.0/2.0;
                appendLabels(primitives[i].text, midPointAngleOut(primitives[i].arr0, primitives[i].arr1, primitives[i].arr2, primitives[i].Rad, primitives[i].ratio), 'cc');
            } else if (primitives[i].class==="circle") {
                appendLabels(primitives[i].text, primitives[i].arr0, 'cc');
            } else if (primitives[i].class==="plane") {
                if (!primitives[i].hasOwnProperty('ratio')) primitives[i].ratio = 1;
                if (!primitives[i].hasOwnProperty('Rad')) primitives[i].Rad = 1;
                appendLabels(primitives[i].text, midPointAngle(primitives[i].arr0, primitives[i].arr1, primitives[i].arr3, primitives[i].Rad, primitives[i].ratio), 'cc');
            } else if (primitives[i].class==="triangle") {
...
function drawScene() {
    ...
    mat4.rotateZ(mvMatrix, degToRad(rotAngZ));
