...
function initDescr() {
    var descr = '';
    descr += '<p>Каноническое уравнение параболы:$$y_2=2px$$</p>';
    descr += '<p>$F$ - фокус параболы.<br>';
    descr += 'Прямая $d$ - директриса.</p>';
    descr += '<label><input type="checkbox" checked onchange="isShowC = this.checked; initBuffers();"> $c=\\frac p 2$ - расстояние до фокуса.</label>';
    descr += '<p>$\\varepsilon=1$ - эксцентриситет.</p>';
    descr += '<p>$|MB|$ - расстояние от точки $M$ до директрисы $d$.</p>';
    descr += '<p>$|MF|=|MB|$</p>';
...
function initData() {
    ...
    if (isShowC) {
        primitives.push({class:"dashline", text: "c", pos: 'lt', arr0:[0,0,0], arr1:[p/2,0,0], rad:arrRad*1.2, color:[0.0, 0.4, 0.8, 1.0]});
    }
    primitives.push({class:"line", text: "p", ratio:0.3, arr0:[-p/2,0,0], arr1:[p/2,0,0], rad:arrRad, color:[0.0, 0.7, 0.0, 1.0]});
    primitives.push({class:"line", text: katex.renderToString('d'), ratio: 0.7, arr0:[-p/2,-maxLen0,0], arr1:[-p/2,maxLen0,0], rad:arrRad, color:[0.0, 1.0, 0.0, 1.0]});
    primitives.push({class:"point", text: katex.renderToString('B'), arr0:[-p/2,pointM[1],0], rad:pointRad, color:[0.0, 0.6, 0.0, 1.0]});
