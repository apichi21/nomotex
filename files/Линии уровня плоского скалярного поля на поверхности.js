...
function initPoints() {
    ...
    */
    ...
    points.push({coord1: vec3.create([xMin, yMin, uxy.eval({x: xMin, y: yMin})]), movable: "free"});
    points.push({coord1: vec3.create([xMax, yMax, uxy.eval({x: xMax, y: yMax})]), movable: "free"});
    points.push({coord1: vec3.create([xMinC, yMinC, uxy.eval({x: xMinC, y: yMinC})]), movable: "free"});
    points.push({coord1: vec3.create([xMaxC, yMaxC, uxy.eval({x: xMaxC, y: yMaxC})]), movable: "free"});
}
...
function initDescr() {
    ...
    */
    $("#description").html(`<p style="font-size: ${descriptionFontSize}px">Линии уровня плоского скалярного поля на поверхности.<br/></p>`);
    ...
    <button id="buildchart" style="margin: 15px auto;display: block;">Построить</button><br/>
    `;
...
function initData() {
    ...
    if (points[0].coord1[0] > points[1].coord1[0] - stepX) {
        ...
        points[0].coord1[2] = uxy.eval({x: points[0].coord1[0], y: points[0].coord1[1]})
    }
    ...
    if (points[2].coord1[0] > points[3].coord1[0] - stepXC) {
        ...
        points[2].coord1[2] = uxy.eval({x: points[2].coord1[0], y: points[2].coord1[1]})
    }
    ...
    */
    primitives.push({
        class: "point",
        text: `<span style="font-size: 0.75em">П: ${katex.renderToString('(x_{min},y_{min})')}</span> `,
        pos: "rt",
    ...
    primitives.push({
        class: "point",
        text: `<span style="font-size: 0.75em">П: ${katex.renderToString('(x_{max},y_{max})')}</span> `,
    ...
    primitives.push({
        class: "point",
        text: `<span style="font-size: 0.75em">ЛУ: ${katex.renderToString('(x_{min},y_{min})')}</span> `,
        pos: "rt",
    ...
    primitives.push({
        class: "point",
        text: `<span style="font-size: 0.75em">ЛУ: ${katex.renderToString('(x_{max},y_{max})')}</span> `,
    ...
    for (let x = xMin; x <= xMax - stepX; x += stepX) {
        for (let y = yMin; y <= yMax - stepY; y += stepY) {
            primitives.push({
                ...
                arr2: [x + stepX, y + stepY, uxy.eval({x: x + stepX, y: y + stepY})],
