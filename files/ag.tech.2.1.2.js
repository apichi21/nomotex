...
function initPoints() {
    ...
    points.push({coord1: vec3.create([-1, -1.8, 0]), movable: "plane", vector:[0,1,0]});
}
var meshPlane = {}, meshRadar = {};
function initDescr() {
    var descr = "Радиолокационная станция определяет радиус-вектор $\\vec a$ самолёта в момент времени $t$.";
...
var te5 = katex.renderToString("\\vec a");
...
function initData() {
    ...
    if (meshRadar.isready)
    {
        ...
        meshes.push({
            ...
            normals:normalCoords,
            indices:vertexIndices,
            rotateY:findRotateY(points[1].coord1, points[0].coord1),
    ...
    primitives.push({class:"arrow", text: te5, arr0:points[1].coord1, arr1:points[0].coord1, offset:true, rad:2, color:[0.0, 0.8, 0.0, 1.0]});
    var plen = 5;
    var pz = -3;
