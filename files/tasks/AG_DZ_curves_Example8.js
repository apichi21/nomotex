...
function initDescr() {
    ...
    var conditions = "$а)$ Определить тип кривой по заданному уравнению вида \\begin{equation} Ax^2+2Вxy+Cy^2+2Dx+2Ey+F=0, \\end{equation} где $A, B, C, D, E, F$ - некоторые действительные числа, а также определить тип кривой для частного случая из пункта $б)$.<br> \
      $б)$ Привести кривую к каноническому виду и записать название кривой для случая, когда $A=0$, $B=0$, $C=9$, $D=0$, $E=-\\frac{7}{2}$, $F=-16$, т.е. уравнение $(1)$ принимает вид: \\begin{equation*} 9y^2 -7y -16 = 0\\end{equation*}\
    ...
    if (showAlgorithm) {
      ...
      \\begin{equation*} I = \\begin{vmatrix} 0 & 0 \\\\ 0 & 9 \\end{vmatrix} = 0. \\end{equation*}\
      Следовательно, кривая параболического типа.</p>";
      algorithm += "<p class='toggleable'>$б)$ Для приведения кривой к каноническому виду необходимо выделить полныЙ квадрат, для этого группируем члены, содержащие $y$: \
      \\begin{equation*} 9(y^2 -\\frac{7}{9}y+\\frac{49}{324}-\\frac{49}{324}) -16=0; \\end{equation*}\
      \\begin{equation*} 9(y^2 -\\frac{7}{18})^2-\\frac{49}{36} -16=0; \\end{equation*}\
      \\begin{equation*} 9(y -\\frac{7}{18})^2-\\frac{625}{36}=0; \\end{equation*}\
      \\begin{equation} (y -\\frac{7}{18})^2=\\frac{625}{324}; \\end{equation}\ </p>";
      algorithm += "<p class='toggleable'>Сделаем замену координат: \
      \\begin{equation*} y'=y -\\frac{7}{18} \\end{equation*}\</p>";
      algorithm += "<p class='toggleable'>Тогда уравнение $(2)$ примет вид: \
      \\begin{equation*}  y'^2=a^2 \\end{equation*} \
      &mdash; каноническое уравнение пары параллельных прямых, где $a=\\frac{25}{18}$. <br> \
      $O'=(0;\\frac{7}{18})$ &mdash; начало канонической системы координат $O'x'y'$.</p>";
      algorithm += "<p class='toggleable'>Следовательно, уравнение $(2)$ является уравнением пары параллельных прямых:\
      \\begin{equation*} y_1=\\frac{25}{18}+ \\frac{7}{18}=\\frac{32}{18}=\\frac{16}{9};\\end{equation*}\
      \\begin{equation*} y_2=-\\frac{25}{18}+ \\frac{7}{18}=-\\frac{18}{18}=-1\\end{equation*} </p>";
      algorithm += "<p class='toggleable'>$в)$отсутствуют</p>";
    ...
    values[0].push([0, 0, 9, 0, -7, -16]);
    values[0].push([5, 0, 0, 10, 0, -15]);
    values[0].push([0, 0, 2, 0, 10, 4]);
