...
function initDescr() {
    ...
    var conditions = "$а)$ Определить тип кривой по заданному уравнению вида \\begin{equation} Ax^2+2Вxy+Cy^2+2Dx+2Ey+F=0, \\end{equation} где $A, B, C, D, E, F$ - некоторые действительные числа, а также определить тип кривой для частного случая из пункта $б)$.<br> \
      $б)$ Привести кривую к каноническому виду и записать название кривой для случая, когда $A=4$, $B=0$, $C=9$, $D=6$, $E=-3$, $F=10$, т.е. уравнение $(1)$ принимает вид: \\begin{equation*} 4 x^2 + 9 y^2 + 12 x -6 y + 10 = 0\\end{equation*}\
    ...
    if (showAlgorithm) {
      ...
      \\begin{equation*} I = \\begin{vmatrix} 4 & 0 \\\\ 0 & 9 \\end{vmatrix} = 36. \\end{equation*}\
      ...
      \\begin{equation*} 4\\left( x^2 + 3x + \\frac{9}{4} - \\frac{9}{4} \\right) + 9\\left(y^2 - \\frac{2}{3}y + \\frac{1}{9}- \\frac{1}{9}\\right)+10=0; \\end{equation*}\
      \\begin{equation*} 4\\left(x+\\frac{3}{2}\\right)^2 + 9\\left(y-\\frac{1}{3}\\right)^2 -9-1+10=0; \\end{equation*}\
      \\begin{equation*} 4\\left(x+\\frac{3}{2}\\right)^2 + 9\\left(y-\\frac{1}{3}\\right)^2 = 0; \\end{equation*}\
      \\begin{equation} \\frac{(x+\\frac{3}{2})^2}{9} + \\frac{(y-\\frac{1}{3})^2}{4} = 0. \\end{equation} </p>";
      algorithm += "<p class='toggleable'>Сделаем замену координат: \
      \\begin{equation*} \\begin{cases} x'= x + \\frac{3}{2} \\\\ y'= y - \\frac{1}{3} \\end{cases} \\end{equation*}</p>";
      algorithm += "<p class='toggleable'>Тогда уравнение $(2)$ примет вид: \
      \\begin{equation*}  \\frac{x'^2}{a^2}+ \\frac{y'^2}{b^2}=0  \\end{equation*} \
      &mdash; каноническое уравнение точки (вырожденного эллипса, пары мнимых пересекающихся прямых), где $a=3$, $b=2$. <br> \
      $O'=(-\\frac{3}{2}; \\frac{1}{3})$ &mdash; начало канонической системы координат $O'x'y'$.</p>";
      algorithm += "<p class='toggleable'>Следовательно, уравнение $(2)$ является уравнением точки  $O'=(-\\frac{3}{2}; \\frac{1}{3})$. </p>";
