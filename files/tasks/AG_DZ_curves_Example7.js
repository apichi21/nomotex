...
function initDescr() {
    ...
    var conditions = "$а)$ Определить тип кривой по заданному уравнению вида \\begin{equation} Ax^2+2Вxy+Cy^2+2Dx+2Ey+F=0, \\end{equation} где $A, B, C, D, E, F$ - некоторые действительные числа, а также определить тип кривой для частного случая из пункта $б)$.<br> \
      $б)$ Привести кривую к каноническому виду и записать название кривой для случая, когда $A=25$, $B=0$, $C=0$, $D=0$, $E=-15$, $F=9$, т.е. уравнение $(1)$ принимает вид: \\begin{equation*} 25 x^2 - 30 x +9 = 0\\end{equation*}\
    ...
    if (showAlgorithm) {
      ...
      \\begin{equation*} I = \\begin{vmatrix} 25 & 0 \\\\ 0 & 0 \\end{vmatrix} = 0. \\end{equation*}\
      Следовательно, кривая параболического типа.</p>";
      algorithm += "<p class='toggleable'>$б)$ Для приведения кривой к каноническому виду необходимо выделить полные квадраты, для этого группируем члены, содержащие $x$: \
      \\begin{equation*} 25(x^2 -\\frac{6}{5}x + \\frac{9}{25} -\\frac{9}{25}) +9=0; \\end{equation*}\
      \\begin{equation*} 25(x -\\frac{3}{5})^2-9 +9=0; \\end{equation*}\
      \\begin{equation} 25(x -\\frac{3}{5})^2=0; \\end{equation}\ </p>";
      algorithm += "<p class='toggleable'>Сделаем замену координат: \
      \\begin{equation*}  x'= x - \\frac{3}{5} \\end{equation*}</p>";
      algorithm += "<p class='toggleable'>Тогда уравнение $(2)$ примет вид: \
      \\begin{equation*}  x'^2=0  \\end{equation*} \
      &mdash; каноническое уравнение пары совпавших прямых. <br> \
      $O'=(\\frac{3}{5}; 0)$ &mdash; начало канонической системы координат $O'x'y'$.</p>";
      algorithm += "<p class='toggleable'>Следовательно, уравнение $(2)$ является уравнением пары совпавших прямых:  $x=\\frac{3}{5} $. </p>";
    ...
    values[0].push([25, 0, 0, -30, 0, 9]);
    values[0].push([0, 0, 3, 0, -12, 12]);
    values[0].push([4, 0, 0, 24, 0, 36]);
