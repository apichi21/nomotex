...
function initDescr() {
    ...
    var conditions = "Приведите к каноническому виду уравнение поверхности, заданное в виде \
      ...
      где $a_{ij}$, $b_i$, $c$ &mdash; некоторые действительные числа, $i,j = \\overline{1,3}$, для случая, когда $a_{11}=1$, $a_{22}=2$, $a_{33}=1$, $a_{12}=0$, $a_{13}=0$, $a_{23}=0$, $b_1 = -3$, $b_2 = -2$, $b_3 = -1$, $c=-7$, то есть уравнение $(1)$ принимает вид: \
      \\begin{equation*} x^2 - 6 x + 2 y^2 - 4 y + z^2 - 2 z - 7 = 0.\\end{equation*} Назовите все кривые, полученные в результате сечения поверхности координатыми плоскостями в канонической системе координат.";

    $("#conditions").html('<h3>Условия задачи:</h3>'+conditions);

    if (showAlgorithm) {
      var algorithm = "<p class='toggleable'>Для приведения поверхности к каноническому виду необходимо выделить полные квадраты. Для этого группируем члены, содержащие $x$, $y$, $z$: \
    ...
    values[0].push([1, 2, 1, -6, -4, -2, -7, 0, 0, 0]);
    values[0].push([16, 4, 1, 0, -24, -4, 24, 0, 0, 0]);
    values[0].push([9, 16, 36, 72, 96, 144, 288, 0, 0, 0]);
...
function initData() {
    ...
    if (showSolution) {
        ...
        if (Ax2_1 == 0 && Ay2_1 == 0) {
            ...
            var arrYLen = a[1]*2;
            var arrZLen = a[2]*2;
            primitives.push({class:"arrow", text: "x'", ratio: 1, arr0:[y0, z0, x0-arrXLen], arr1:[y0, z0, x0+arrXLen], rad:1.2, color:[0.5, 0.0, 0.0, 1.0]});
            primitives.push({class:"arrow", text: "y'", ratio: 1, arr0:[y0-arrYLen, z0, x0], arr1:[y0+arrYLen, z0, x0], rad:1.2, color:[0.5, 0.0, 0.0, 1.0]});
            primitives.push({class:"arrow", text: "z'", ratio: 1, arr0:[y0, z0-arrZLen, x0], arr1:[y0, z0+arrZLen, x0], rad:1.2, color:[0.5, 0.0, 0.0, 1.0]});
            primitives.push({class:"point", text: "O'", arr0:[y0, z0, x0], rad:2, color:[0.5, 0.0, 0.0, 1.0]});
