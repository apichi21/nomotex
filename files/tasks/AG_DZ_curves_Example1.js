...
function initDescr() {
    ...
    var conditions = "$а)$ Определить тип кривой по заданному уравнению вида \\begin{equation} Ax^2+2Вxy+Cy^2+2Dx+2Ey+F=0, \\end{equation} где $A, B, C, D, E, F$ - некоторые действительные числа, а также определить тип кривой для частного случая из пункта $б)$.<br> \
      $б)$ Привести кривую к каноническому виду и записать название кривой для случая, когда $A=2$, $B=0$, $C=1$, $D=2$, $E=2$, $F=2$, т.е. уравнение $(1)$ принимает вид: \\begin{equation*} 2 x^2 + y^2 + 4 x+ 4 y + 2 = 0\\end{equation*}\
    ...
    values[0].push([2, 0, 1, 4, 4, 2]);
    values[0].push([9, 0, 25, 54, -100, -44]);
    values[0].push([4, 0, 9, -8, 36, 4]);
    ...
    if (showSolution) {
        ...
        if (A1 == 0 && C1 != 0 && D1 != 0) {
            ...
            if (a>=b) {
                ...
                primitives.push({class:"arrow", text: "y'", ratio: 1, pos: "rb", arr0:[x0-arrXLen, y0, 0], arr1:[x0+arrXLen, y0, 0], rad:1.2, color:[0.5, 0.0, 0.0, 1.0]});
                primitives.push({class:"arrow", text: "x'", ratio: 1, pos: "lt", arr0:[x0, y0-arrYLen, 0], arr1:[x0, y0+arrYLen, 0], rad:1.2, color:[0.5, 0.0, 0.0, 1.0]});
            }
