...
function initDescr() {
    ...
    var conditions = "Приведите к каноническому виду уравнение поверхности, заданное в виде \
      ...
      где $a_{ij}$, $b_i$, $c$ &mdash; некоторые действительные числа, $i,j = \\overline{1,3}$, для случая, когда $a_{11}=1$, $a_{22}=1$, $a_{33}=0$, $a_{12}=0$, $a_{13}=0$, $a_{23}=0$, $b_1 = 1$, $b_2 = -2$, $b_3 = -2$, $c=-13$, то есть уравнение $(1)$ принимает вид: \
      \\begin{equation*} x^2 + 2 x + 2 y^2 - 4 y  - 4 z - 13 = 0.\\end{equation*} Назовите все кривые, полученные в результате сечения поверхности координатыми плоскостями в канонической системе координат.";
    ...
    if (showAlgorithm) {
      ...
      \\begin{equation*}  (x^2 + 2 x + 1 - 1) + 2(y^2 - 2 y + 1 - 1) - 4 z - 13 =0;\\end{equation*} \
      \\begin{equation*} (x+1)^2 + 2(y-1)^2 - 1 - 2 - 13 - 2 z = 0; \\end{equation*}\
      \\begin{equation*} (x+1)^2 + 2(y-1)^2  = 4(z+4). \\end{equation*}\
      \\begin{equation} \\frac{(x+1)^2}{4} + \\frac{(y-1)^2}{2}  = z+4. \\end{equation}</p>";
      algorithm += "<p class='toggleable'>Сделаем замену координат: \
      \\begin{equation*} \\begin{cases} x'= x + 1, \\\\ y' = y - 1, \\\\ z' = z + 4.  \\end{cases} \\end{equation*} </p>";
      algorithm += "<p class='toggleable'>Тогда уравнение $(2)$ примет вид: \
      \\begin{equation} \\frac{x'^2}{a^2} + \\frac{y'^2}{b^2}  = z' \\end{equation} \
      &mdash; каноническое уравнение эллиптического параболоида, где $a=2$, $b=\\sqrt 2$. $O' = (-1, 1, -4)$  &mdash; начало канонической системы координат $O'x'y'z'$. </p>";
      algorithm += "<p class='toggleable'>Следовательно, уравнение $(2)$ является уравнением эллипического параболоида с вершиной $O'= (-1, 1, -4)$. </p>";
      algorithm += "<p class='toggleable'>При сечении поверхности, заданной уравнением $(3)$ координатными плоскостями: <br> \
      $a)$ $x'=0$, имеем \
      \\begin{equation*} y'^2  = b^2 z'  \\end{equation*} \
     &mdash; каноническое уравнение параболы ($p=\\frac{b^2}{2}$). <br>  \
      $б)$ $y'=0$, имеем \
      \\begin{equation*} x'^2  = a^2 z'  \\end{equation*} \
      &mdash; каноническое уравнение параболы ($p=\\frac{a^2}{2}$).<br>  \
      $в)$ $z'=0$, имеем \
    ...
    values[0].push([1, 2, 0, 2, -4, -4, -13, 0, 0, 0]);
    values[0].push([1, 0, 1, 0, -1, -4, 7, 0, 0, 0]);
    values[0].push([9, 4, 0, -54, 40, 36, 145, 0, 0, 0]);
    var variants = '';
