...
function initDescr() {
    ...
    var conditions = "$а)$ Определить тип кривой по заданному уравнению вида \\begin{equation} Ax^2+2Вxy+Cy^2+2Dx+2Ey+F=0, \\end{equation} где $A, B, C, D, E, F$ - некоторые действительные числа.<br> \
      $б)$ Привести кривую к каноническому виду и записать название кривой.<br>\
    ...
    if (showAlgorithm) {
      ...
      algorithm += "<p class='toggleable'>Рассмотрим случай, когда $A=1$, $B=0$, $C=0$, $D=3$, $E=-8$, $F=25$, т.е. уравнение $(1)$ принимает вид: \\begin{equation*} x^2 + 6 x - 16 y + 25 = 0\\end{equation*}</p>";
      algorithm += "<p class='toggleable'>$a)$ Для определения типа кривой с уравнением $(1)$ вычислим определитель $I$, составленный из коэффициентов уравнения $(1)$: \
      ...
      \\begin{equation*} I = \\begin{vmatrix} 1 & 0 \\\\ 0 & 0 \\end{vmatrix} = 0. \\end{equation*}\
      Следовательно, кривая параболического типа.</p>";
      ...
      \\begin{equation*} (x^2 + 6x + 9 -9) -16y +25=0; \\end{equation*}\
      \\begin{equation*} (x^2 + 6x + 9) -16y +25-9=0; \\end{equation*}\
      \\begin{equation} (x+3)^2=16(y-1); \\end{equation}\ </p>";
      algorithm += "<p class='toggleable'>Сделаем замену координат: \
      \\begin{equation*} \\begin{cases} x'=y-1 \\\\ y'=x+3 \\end{cases} \\end{equation*}</p>";
      algorithm += "<p class='toggleable'>Тогда уравнение $(2)$ примет вид: \
      \\begin{equation*}  y'^2=2px'  \\end{equation*} \
      &mdash; каноническое уравнение параболы, где $p=8$ - параметр параболы. <br> \
      $O'=(-3; 1)$ &mdash; начало канонической системы координат $O'x'y'$.</p>";
      algorithm += "<p class='toggleable'>Следовательно, уравнение $(2)$ является уравнением эллипса с вершиной $O'=(-3; 1)$,параметром $p=8$. </p>";
      algorithm += "<p class='toggleable'>$в)$Известно, что координаты фокуса в канонической системе координат $O'x'y'$ имеют вид: \
      \\begin{equation*} F'= (\\frac{p}{2},0).\\end{equation*} \
      Следовательно,\
      \\begin{equation*} F'= (4,0). \\end{equation*}      </p>";
      algorithm += "<p class='toggleable'>А в системе координат $Oxy$: \
      \\begin{equation*} F= (-3, 1+\\frac{p}{2})=(-3,5). \\end{equation*} </p>";
      algorithm += "<p class='toggleable'>Найдем уравнение директрисы $d'$ параболы в системе координат $O'x'y'$: \
      \\begin{equation*} d':x'= -\\frac{p}{2}=-4 \\end{equation*}</p>";
      algorithm += "<p class='toggleable'>Тогда в системе координат $Oxy$ уравнение директрисы $d$ имеет вид: \
      \\begin{equation*} d:y-1= -4 \\end{equation*}\
      \\begin{equation*} d:y= -3 \\end{equation*}</p>";
    ...
    // values[0].push([0, 0, 1, -8, 6, 17]);
    // values[0].push([3, 0, 0, 12, 12, 0]);
    // values[0].push([10, 0, 0, -20, -1, 7]);
    // values[0].push([0, 0, 1, -3, -10, 34]);
    // values[0].push([16, 0, 0, -32, -1, 15]);
    // values[0].push([0, 0, 15, -1, -30, 10]);
    // values[0].push([0, 0, 1, 9, -14, 85]);
    // values[0].push([1, 0, 0, -24, 4, 140]);
    // values[0].push([0, 0, 1, -1, -8, 13]);
    // values[0].push([0, 0, 9, 4, -12, 28]);
    // values[0].push([1, 0, 0, -16, -8, 8]);
    // values[0].push([0, 0, 1, -10, -8, 26]);//
    // values[0].push([30, 0, 0, -60, -1, 29]);
    // values[0].push([0, 0, 3, -2, 6, 1]);
    // values[0].push([1, 0, 0, 16, -2, 66]);
    // values[0].push([0, 0, 4, -16, -4, 17]);
    // values[0].push([1, 0, 0, -18, 8, 73]);//
    // values[0].push([5, 0, 0, -20, 2, 8]);
    // values[0].push([0, 0, 2, -1, -44, 250]);
    // values[0].push([7, 0, 0, -14, 2, 11]);
    // values[0].push([0, 0, 2, -7, 16, 25]);
    // values[0].push([4, 0, -25, 4, 50, 76]);
    // values[0].push([100, 0, -36, 300, -180, -225]);
    // values[0].push([4, 0, -9, -8, -72, 4]);
    // values[0].push([8, 0, -9, 32, 90, -121]);
    // values[0].push([2, 0, -1, -12, 4, -2]);
    // values[0].push([4, 0, -1, 24, -10, 47]);
    // values[0].push([36, 0, -25, -144, 150, 819]);
    // values[0].push([4, 0, -36, 4, 36, -17]);
    // values[0].push([9, 0, -25, 0, 50, 200]);

    values[0].push([0, 0, 1, -8, 6, 17]);
    values[0].push([3, 0, 0, 12, 12, 0]);
    values[1].push([10, 0, 0, -20, -1, 7]);
    values[1].push([0, 0, 1, -3, -10, 34]);
    values[1].push([16, 0, 0, -32, -1, 15]);
    values[1].push([0, 0, 15, -1, -30, 10]);
    values[1].push([0, 0, 1, 9, -14, 85]);
    values[1].push([1, 0, 0, -24, 4, 140]);
    values[1].push([0, 0, 1, -1, -8, 13]);
    values[1].push([0, 0, 9, 4, -12, 28]);
    values[1].push([1, 0, 0, -16, -8, 8]);
    values[1].push([0, 0, 1, -10, -8, 26]);
    values[1].push([30, 0, 0, -60, -1, 29]);
    values[1].push([0, 0, 3, -2, 6, 1]);
    values[1].push([1, 0, 0, 16, -2, 66]);
    values[1].push([0, 0, 4, -16, -4, 17]);
    values[2].push([1, 0, 0, -18, 8, 73]);
    values[2].push([5, 0, 0, -20, 2, 8]);
    values[2].push([0, 0, 2, -1, -44, 250]);
    values[2].push([7, 0, 0, -14, 2, 11]);
    values[2].push([0, 0, 2, -7, 16, 25]);
    values[2].push([4, 0, -25, 4, 50, 76]);
    values[2].push([100, 0, -36, 300, -180, -225]);
    values[2].push([4, 0, -9, -8, -72, 4]);
    values[2].push([8, 0, -9, 32, 90, -121]);
    values[2].push([2, 0, -1, -12, 4, -2]);
    values[2].push([4, 0, -1, 24, -10, 47]);
    values[2].push([36, 0, -25, -144, 150, 819]);
    values[2].push([4, 0, -36, 4, 36, -17]);
    values[2].push([9, 0, -25, 0, 50, 200]);
