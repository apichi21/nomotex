...
function initDescr() {
    ...
    var conditions = "$а)$ Определить тип кривой по заданному уравнению вида \\begin{equation} Ax^2+2Вxy+Cy^2+2Dx+2Ey+F=0, \\end{equation} где $A, B, C, D, E, F$ - некоторые действительные числа, а также определить тип кривой для частного случая из пункта $б)$.<br> \
      $б)$ Привести кривую к каноническому виду и записать название кривой для случая, когда $A=1$, $B=0$, $C=0$, $D=3$, $E=-8$, $F=25$, т.е. уравнение $(1)$ принимает вид: \\begin{equation*} x^2 + 6 x - 16 y + 25 = 0\\end{equation*}\
    ...
    if (showAlgorithm) {
      ...
      algorithm += "<p class='toggleable'>Следовательно, уравнение $(2)$ является уравнением параболы с вершиной $O'=(-3; 1)$,параметром $p=8$. </p>";
...
function initData() {
    ...
    if (showSolution) {
        ...
        if (A1 == 0 && C1 != 0 && D1 != 0) {
            ...
            var arrXLen = Math.abs(p)/2*1.5;
            var arrYLen = Math.abs(p)/2*1.5;
