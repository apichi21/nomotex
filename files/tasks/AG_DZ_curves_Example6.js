...
function initDescr() {
    ...
    var conditions = "$а)$ Определить тип кривой по заданному уравнению вида \\begin{equation} Ax^2+2Вxy+Cy^2+2Dx+2Ey+F=0, \\end{equation} где $A, B, C, D, E, F$ - некоторые действительные числа, а также определить тип кривой для частного случая из пункта $б)$.<br> \
      $б)$ Привести кривую к каноническому виду и записать название кривой для случая, когда $A=2$, $B=0$, $C=1$, $D=2$, $E=-3$, $F=12$, т.е. уравнение $(1)$ принимает вид: \\begin{equation*} 2 x^2 + y^2 + 4 x -6 y + 12 = 0\\end{equation*}\
    ...
    if (showAlgorithm) {
      ...
      \\begin{equation*} 2\\left( x^2 + 2x + 1 - 1 \\right) + \\left(y^2 - 6y + 9 - 9 \\right)+12=0; \\end{equation*}\
      \\begin{equation*} 2( x + 1 )^2 + (y - 2  )^2 - 2- 9+12=0; \\end{equation*}\
      \\begin{equation*} 2( x + 1 )^2 + (y - 2  )^2 + 1 =0;  \\end{equation*}\
      \\begin{equation} \\frac{(x+1)^2}{\\frac{1}{2}} + \\frac{(y-3)^2}{1} = -1. \\end{equation} </p>";
      algorithm += "<p class='toggleable'>Сделаем замену координат: \
      \\begin{equation*} \\begin{cases} x'= x + 1 \\\\ y'= y - 3 \\end{cases} \\end{equation*}</p>";
      algorithm += "<p class='toggleable'>Тогда уравнение $(2)$ примет вид: \
      \\begin{equation*}  \\frac{x'^2}{a^2}+ \\frac{y'^2}{b^2}= -1  \\end{equation*} \
      &mdash; каноническое уравнение мнимого эллипса, где $a=\\frac{1}{\\sqrt{2}}$, $b=1$. <br> \
      $O'=(-1; 3)$ &mdash; начало канонической системы координат $O'x'y'$.</p>";
      algorithm += "<p class='toggleable'>Следовательно, уравнение $(2)$ является уравнением мнимого эллипса. </p>";
