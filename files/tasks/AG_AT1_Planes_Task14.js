...
function initDescr() {
    ...
    var conditions = "Составьте уравнение прямой $l_2$, проходящей через точку пересечения $P$ прямой\
      ...
      \\begin{equation} \\pi_2:A_2x+B_2y+C_2z+D_2=0\\end{equation}";
    ...
    if (showAlgorithm) {
      ...
      algorithm += "<p>Составим канонические уравнения прямой $l_2$, учитывая условие перпендикулярности плоскости $\\pi_2$\
      (вектор нормали $\\vec n_2=\\{A_2;B_2;C_2\\}$ к плоскости $\\pi_2$ является направляющим вектором прямой $l_2$):\
      \\begin{equation}\\frac{x-x_0}{A_2}=\\frac{y-y_0}{B_2}=\\frac{z-z_0}{C_2}\\end{equation}</p>";
    ...
    values[0].push([1,-1,0, 2,3,-1, 2,1,-2,-10, 1,2,-3,-77]);
    values[0].push([4,1,-4,3,-2,-5,-4,-1,1,-9,2,0,2,-2]);
    values[1].push([-5,-3,-1,4,-1,1,-4,1,-3,0,0,-3,-1,1]);
    values[1].push([-4,-4,-2,2,4,-3,2,-2,2,-6,-1,-3,-1,1]);
    values[1].push([-4,2,-1,0,-3,0,0,-4,1,9,2,2,-2,6]);
    values[1].push([-1,-4,1,-1,-1,-5,-3,0,2,9,1,-1,1,3]);
    values[1].push([4,2,4,-2,-3,2,0,1,-2,6,3,-3,-1,5]);
    values[1].push([-3,4,3,-4,-5,1,3,0,-1,-1,3,1,2,-9]);
    values[1].push([0,-2,1,2,2,-1,-4,0,-2,-10,1,3,-3,4]);
    values[1].push([-1,1,-4,-2,-1,4,1,-2,-1,-5,0,0,1,-9]);
    values[1].push([2,-2,-4,0,-5,-3,-1,1,0,4,3,1,3,-9]);
    values[1].push([-1,-4,-1,4,-4,-4,-2,0,-3,-1,0,-4,-2,5]);
    values[1].push([-3,-4,0,4,4,0,0,2,-2,-8,2,3,-2,-7]);
    values[1].push([3,3,-1,1,2,1,-4,-4,3,-9,-4,1,1,-10]);
    values[1].push([-3,1,2,-3,-3,-3,2,1,2,-4,-4,2,0,-4]);
    values[1].push([-2,2,-3,-4,4,0,3,-3,-1,-3,3,-3,-4,3]);
    values[2].push([-2,-3,-4,0,-2,4,-3,1,2,5,2,-1,0,-7]);
    values[2].push([-1,1,-3,0,3,-1,-3,-1,-4,-10,-3,2,3,-7]);
    values[2].push([2,-4,3,2,-1,1,-3,-4,-4,-10,0,1,1,-4]);
    values[2].push([-1,-4,2,4,-2,4,1,-4,2,1,-3,2,0,-9]);
    values[2].push([1,1,4,3,1,1,1,2,-3,3,-1,-1,1,9]);
    values[2].push([2,3,2,3,3,-3,3,0,2,-6,-1,-2,0,-5]);
    values[2].push([4,4,-4,0,0,-5,-3,1,1,3,0,3,-2,0]);
    values[2].push([-2,-5,-2,3,-3,-3,-1,-2,-1,8,0,3,-2,5]);
    values[2].push([1,3,-2,2,-3,1,-1,-2,-3,3,-2,0,-3,7]);
    values[2].push([-2,-1,-2,-1,-4,-5,-3,-3,1,3,1,-3,3,1]);
    values[2].push([-3,3,4,-5,4,2,3,0,2,1,0,0,-1,-1]);
    values[2].push([-1,-3,1,-2,2,0,-2,-1,-2,-1,-2,-2,2,8]);
    values[2].push([3,4,-1,4,2,-4,-2,2,0,0,2,-1,-1,9]);
    values[2].push([-2,4,-4,0,3,-3,2,0,-1,1,-2,0,-3,1]);
    ...
    variants += '<p>$\\pi_2$: <span id="pi2"></span></p>';
...
function changeVariant(newVar) {
    ...
    var i = 10;
    $("#pi2").html( "$" +linCombText([cval[i],cval[i+1],cval[i+2],cval[i+3]],["x","y","z",""]) + "=0$" );
    for (var i = 0; i < 3; i++) {
        p1[i] = cval[i];
        vecS1[i] = cval[i+3];
    }
...
function initData() {
    ...
    if (showSolution) {
        ...
        $("#ans4").val(parseFloat(abcd2[0].toFixed(precision1)));
        $("#ans5").val(parseFloat(abcd2[1].toFixed(precision1)));
        $("#ans6").val(parseFloat(abcd2[2].toFixed(precision1)));

        var ansp2 = [];
        vec3.add(intersect, abcd2, ansp2);
        var leftPoint = [];
        var rightPoint = [];
        createLine(intersect,ansp2,leftPoint,rightPoint,30);
