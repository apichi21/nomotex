...
function initDescr() {
	$("#description").html('<div class="header">Выражение главного значения аргумента через арктангенс.</div>');
...
function initData() {
	...
	switch (quarterType) {
		case 'quarter-1':
			...
			let pa = [coneX0 - baseLen * Math.cos(anglePer), coneY0 - baseLen * Math.sin(anglePer), 0.0],
				pb = [coneX0 + baseLen * Math.cos(anglePer), coneY0 + baseLen * Math.sin(anglePer), 0.0];
