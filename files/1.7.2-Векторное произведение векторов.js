...
function initDescr() {
    ...
    textInputId = 12;
    parametershtml += "$\\vec c=\\vec a\\times\\vec b$: x<input type='text' id='x"+textInputId+"' size='"+textInputSize+"'readonly> y<input type='text' id='y"+textInputId+"' size='"+textInputSize+"'readonly> z<input type='text' id='z"+textInputId+"' size='"+textInputSize+"'readonly> <br/>";
    parametershtml += "$|\\vec c|=|\\vec a||\\vec b|\\sin(\\varphi) = $<input type='text' id='C' size='"+textInputSize+"'readonly> <br/>";
    ...
    $("#description").html("Векторное произведение векторов $\\vec a$ и $\\vec b.$");
...
function initData() {
    ...
    var crossablen = vec3.length(crossab);
    ...
    primitives.push({class:"arc", text: katex.renderToString("\\varphi"), arr0:points[0].coord1, arr1:points[1].coord1, arr2:points[2].coord1, Rad:1.5, rad:1, color:[0.0, 0.0, 1.0, 1.0]});

    if (arrPoint != 0) {
        primitives.push({class:"point", text: "", arr0:arrPoint, rad:5, color:[1.0, 0.0, 1.0, 1.0]}); 
    }

    $("#C").val(parseFloat(crossablen.toPrecision(3)));
    for (var i = 0; i < 4; i++) {
        if (i==0) {
